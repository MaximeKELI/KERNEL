...
void tcp_cc_init(tcp_conn_t* conn) {
    tcp_cc_set_cwnd(conn, TCP_INITIAL_CWND, TCP_INITIAL_SSTHRESH, TCP_WINDOW_SIZE, 0, 0, 0);
}

u32 tcp_cc_snd_wnd(tcp_conn_t* conn) {
    ...
    (void)last_ack;
    (void)dupacks;
    return peer_rwnd < cwnd ? peer_rwnd : cwnd;
}

void tcp_cc_on_ack(tcp_conn_t* conn, u32 ack, u32 acked_bytes) {
    u32 cwnd, ssthresh, peer_rwnd, flight, last_ack;
    u8 dupacks;
    tcp_cc_get(conn, &cwnd, &ssthresh, &peer_rwnd, &flight, &last_ack, &dupacks);
    ...
    if (acked_bytes > 0) {
        if (cwnd < ssthresh) {
            ...
            cwnd += (TCP_MSS * TCP_MSS) / (cwnd ? cwnd : TCP_MSS);
        }
...
void tcp_cc_update_peer_wnd(tcp_conn_t* conn, u32 peer_wnd) {
...
int tcp_cc_parse_sack(tcp_conn_t* conn, const u8* opts, u32 optlen) {
    (void)conn;
    (void)opts;
    (void)optlen;
    return 0;
