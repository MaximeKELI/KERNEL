#include "test.h"
#include "stdio.h"
#include "appliance_config.h"
#include "fs/vfs.h"
#include "ebpf.h"
#include "smp.h"

static test_result_t test_config(void) {
    appliance_config_init();
    const appliance_config_t* c = appliance_config_get();
    return c && c->http_port == 80 ? TEST_PASS : TEST_FAIL;
}

static test_result_t test_tmpfs(void) {
    char buf[16];
    if (vfs_write_path("/tmp/roadmap_test", "ok", 2) < 0) {
        return TEST_FAIL;
    }
    ssize_t n = vfs_read_path("/tmp/roadmap_test", buf, sizeof(buf));
    return n == 2 && buf[0] == 'o' ? TEST_PASS : TEST_FAIL;
}

static test_result_t test_smp(void) {
    return smp_get_cpu_count() >= 1 ? TEST_PASS : TEST_FAIL;
}

static test_result_t test_ebpf(void) {
    typedef struct { u8 opcode; u8 dst; u8 src; i16 off; i32 imm; } insn_t;
    insn_t prog[] = {
        {0x04, 0, 0, 0, 10},
        {0x95, 0, 0, 0, 0},
    };
    u64 r = ebpf_execute_prog(prog, sizeof(prog), 5);
    return r == 15 ? TEST_PASS : TEST_FAIL;
}

void register_roadmap_tests(void) {
    test_register("roadmap", "config", test_config);
    test_register("roadmap", "tmpfs", test_tmpfs);
    test_register("roadmap", "smp", test_smp);
    test_register("roadmap", "ebpf", test_ebpf);
}
