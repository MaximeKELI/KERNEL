...
void ai_sched_benchmark(void) {
    if (!ai_is_enabled()) {
        printk("[bench-ai] Run init-full first (AI not initialized)\n");
        return;
    }
    ...
    printk("\n[bench-ai] I/O scheduler latency (%u iterations)\n",
           BENCH_ITERATIONS);
    ...
    if (with < without) {
        printk("  Gain:    %llu ticks (~%llu ms faster)\n",
               (unsigned long long)(without - with),
               (unsigned long long)delta_ms);
    } else {
        printk("  Gain:    (no improvement this run)\n");
    }
    printk("\n");
}
